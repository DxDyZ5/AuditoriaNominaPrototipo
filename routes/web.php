...
Route::middleware('verified')->group(function () {
    ...
    Route::post('/empleados/guardar', 'EmpleadosController@guardar')->name('empleados.guardar');
    Route::post('/empleados/{empleado}/subir', 'EmpleadosController@subirDocumento')->name('empleados.subir');

    Route::get('/puestos', 'PuestosController@index')->name('puestos');
    Route::post('/puestos/guardar', 'PuestosController@guardar')->name('puestos.guardar');

    Route::get('/documentos', 'DocumentosController@index')->name('documentos');
    Route::post('/documentos/guardar', 'DocumentosController@guardar')->name('documentos.guardar');

    Route::get('/auditorias', 'AuditoriasController@index')->name('auditorias');
    Route::post('/auditorias/{empleado}/ejecutar', 'AuditoriasController@auditarEmpleado')->name('auditorias.ejecutar');

    Route::get('/hallazgos', 'HallazgosController@index')->name('hallazgos');
    Route::post('/hallazgos/{hallazgo}/cerrar', 'HallazgosController@cerrar')->name('hallazgos.cerrar');

    Route::post('/importacion/empleados', 'ImportacionController@empleados')->name('importacion.empleados');

    Route::get('/nomina/carga', 'NominaController@carga')->name('nomina.carga');
    Route::post('/nomina/importar', 'NominaController@importar')->name('nomina.importar');

    Route::get('/finanzas/panel', 'AuditoriaFinancieraController@panel')->name('finanzas.panel');
    Route::post('/finanzas/ejecutar', 'AuditoriaFinancieraController@ejecutar')->name('finanzas.ejecutar');
    Route::view('/finanzas/reglas', 'finanzas.reglas')->name('finanzas.reglas');
}); 
// Report routes
