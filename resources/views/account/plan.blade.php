        ...
        <form action="{{ route('account.plan') }}" method="post" enctype="multipart/form-data">
            ...
            <div class="row m-n2">
                <div class="col-12 col-md-6 p-2 d-flex align-items-center">
                    ...
                    <div class="{{ ($user->plan->features->reports == 0 ? 'text-muted' : '') }} {{ (__('lang_dir') == 'rtl' ? 'mr-2' : 'ml-3') }}">
                        @if($user->plan->features->reports < 0)
                            {{ __('Unlimited reports') }}
                        @else($user->plan->features->reports)
                            <span class="text-muted">{{ number_format($reportsCount, 0, __('.'), __(',')) }} /</span> {{ __(($user->plan->features->reports == 1 ? ':number report' : ':number reports'), ['number' => number_format($user->plan->features->reports, 0, __('.'), __(','))]) }} <span class="text-muted">/ {{ mb_strtolower(__('Month')) }}</span>
                        @endif
                    </div>
                </div>

                <div class="col-12 col-md-6 p-2 d-flex align-items-center">
                    @if($user->plan->features->branded_reports)
                        @include('icons.checkmark', ['class' => 'flex-shrink-0 text-success fill-current width-4 height-4'])
                    @else
                        @include('icons.close', ['class' => 'flex-shrink-0 text-muted fill-current width-4 height-4'])
                    @endif

                    <div class="{{ ($user->plan->features->branded_reports == 0 ? 'text-muted' : '') }} {{ (__('lang_dir') == 'rtl' ? 'mr-2' : 'ml-3') }}">
                        {{ __('Branded reports') }}
                    </div>
                ...
                <div class="col-12 col-md-6 p-2 d-flex align-items-center">
                    @if($user->plan->features->white_label_reports)
                        @include('icons.checkmark', ['class' => 'flex-shrink-0 text-success fill-current width-4 height-4'])
                    @else
                        @include('icons.close', ['class' => 'flex-shrink-0 text-muted fill-current width-4 height-4'])
                    @endif

                    <div class="{{ ($user->plan->features->white_label_reports == 0 ? 'text-muted' : '') }} {{ (__('lang_dir') == 'rtl' ? 'mr-2' : 'ml-3') }}">
                ...
                <div class="col-12 col-md-6 p-2 d-flex align-items-center">
                    @if($user->plan->features->tools)
                        @include('icons.checkmark', ['class' => 'flex-shrink-0 text-success fill-current width-4 height-4'])
                    @else
                        @include('icons.close', ['class' => 'flex-shrink-0 text-muted fill-current width-4 height-4'])
                    @endif

                    <div class="{{ ($user->plan->features->tools == 0 ? 'text-muted' : '') }} {{ (__('lang_dir') == 'rtl' ? 'mr-2' : 'ml-3') }}">
                        {{ __('Tools') }}
                    </div>
