            ...
            <div class="row">
                <div class="col-12">
                        ...
                        <div class="col">
                            <div class="d-flex flex-column flex-lg-row justify-content-around w-100">
                                ...
                                @foreach($tools as $tool)
                                    @if(!isset($menuCategoryId) || isset($menuCategoryId) && $menuCategoryId != $tool->category_id)
                                        <div class="border-left-0 border-left-lg border-bottom border-bottom-lg-0"></div>
                                        <a href="#" class="text-truncate text-decoration-none text-secondary d-flex align-items-center justify-content-lg-center font-weight-medium py-3 px-3 cursor-pointer w-100" data-filter-category="{{ __($tool->category_id) }}" data-text-color-active="text-{{ categoryColor($tool->category_id) }}" data-text-color-inactive="text-secondary">
                                            <span class="text-truncate">{{ __(optional($tool->category)->name ?? $tool->category_id) }}</span>

                                            <span class="flex-shrink-0 badge badge-{{ categoryColor($tool->category_id) }} {{ (__('lang_dir' == 'rtl') ? 'mr-2' : 'ml-2') }}">{{ $tools->countBy('category_id')->toArray()[$tool->category_id] }}</span>
                                        </a>
                                    @endif

                                    @php $menuCategoryId = $tool->category_id; @endphp
                                @endforeach
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row m-n2" id="tools">
                        @foreach($tools as $tool)
                            @if(!isset($categoryId) || isset($categoryId) && $categoryId != $tool->category_id)
                                <div class="col-12 p-2 mt-3" data-category-label="{{ __($tool->category_id) }}">
                                    <div class="badge badge-{{ categoryColor($tool->category_id) }}">{{ __(optional($tool->category)->name ?? $tool->category_id) }}</div>
                                </div>
                            @endif

                            <div class="col-12 col-md-6 col-lg-4 p-2" data-tool="{{ __($tool->name) }} {{ __($tool->description) }}" data-tool-parent="{{ __(optional($tool->category)->name ?? $tool->category_id) }}" data-tool-category="{{ __($tool->category_id) }}">
                            ...
                            @php $categoryId = $tool->category_id; @endphp
                        @endforeach
                    </div>
