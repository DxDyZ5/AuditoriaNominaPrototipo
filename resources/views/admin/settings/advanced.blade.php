...
">{{ config('settings.request_proxy') }}</textarea>
                ...
                <div class="tab-pane fade" id="pills-reports" role="tabpanel" aria-labelledby="pills-reports-tab">
                    <div class="form-group">
                        <label for="i-report-guest">{{ __('Guest') }}</label>
                        <select name="report_guest" id="i-report-guest" class="custom-select{{ $errors->has('report_guest') ? ' is-invalid' : '' }}">
                            @foreach([0 => __('Disabled'), 1 => __('Enabled')] as $key => $value)
                                <option value="{{ $key }}" @if ((old('report_guest') !== null && old('report_guest') == $key) || (config('settings.report_guest') == $key && old('report_guest') == null)) selected @endif>{{ $value }}</option>
                            @endforeach
                        </select>
                        @if ($errors->has('report_guest'))
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $errors->first('report_guest') }}</strong>
                            </span>
                        @endif
                        <small class="form-text text-muted">{{ __('Allow guests to generate limited reports.') }}</small>
                    </div>
                </div>
